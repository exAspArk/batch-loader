...
RSpec.describe BatchLoader do
  ...
  describe "#then" do
    it 'allows modifying the end result without adding more loaders' do
      ...
      post1 = Post.new(user_id: user1.id)
      post2 = Post.new(user_id: user2.id)
      result = { user1: post1.user_lazy.then(&:name), user2: post2.user_lazy.then(&:id) }
    ...
    it 'delegates the second then call to the loaded value' do
      ...
      user_lazy_instance = post.user_lazy.then
      expect(user_lazy_instance.then).to eq("Then from User")
    end
  end
end
